...
<template>
  <Page title="Projects">
    <template #actions>
      ...
      <UButton icon="i-lucide-plus" :to="{ name: 'projects-create' }" label="Create Project" />
    </template>
    <UCard>
      <div v-if="isLoading" class="flex justify-center items-center h-24">
        Loading projects...
      </div>
      <ProjectList v-else-if="projects && projects.length" :projects="projects" :is-loading="isLoading" />
