...
class OpenSearchVectorDatabase(VectorDatabase):
    def __init__(self, host: str, use_ssl: bool = True, port: int = 443, is_serverless: bool = True,
                 region: str = 'us-east-1', username: str = None, password: str = None):
        if is_serverless:
            try:
    ...
    def search(self, index_name: str, query_vector: List[float], k: int) -> List[Dict[str, Any]]:
        vector_field = next((field for field, props in
                             self.client.indices.get_mapping(index=index_name)[index_name]['mappings'][
                                 'properties'].items()
    ...
    def batch_insert_chunks(self, index_name: str, chunks: List[str], chunk_embeddings: List[List[float]],
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            logger.info(
                f"Inserted batch {i // batch_size + 1} ({i + 1} to {min(i + batch_size, total_chunks)} of {total_chunks})")
